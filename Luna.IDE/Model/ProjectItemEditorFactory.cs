        ...
        public EnvironmentWindowComponents MakeEditorFor(ProjectItem projectItem)
        {
            ...
            var modelType = types
                .Where(type =>
                    ...
                    type.GetConstructor(new[] { projectItem.GetType() }) != null)
                .FirstOrDefault() ?? throw new ProjectItemEditorFactoryException();
            ...
            var viewModelType = types
                .Where(type =>
                    type.GetCustomAttribute<EditorForAttribute>()?.ProjectItemType == projectItem.GetType() &&
                    type.GetConstructor(new[] { model.GetType() }) != null)
                .FirstOrDefault() ?? throw new ProjectItemEditorFactoryException();
            ...
            var viewType = types
                .Where(type =>
                    type.GetCustomAttribute<EditorForAttribute>()?.ProjectItemType == projectItem.GetType() &&
                    type.GetConstructor(new[] { viewModel.GetType() }) != null)
                .FirstOrDefault() ?? throw new ProjectItemEditorFactoryException();
