...
namespace Luna.Parsing
{
    public class FunctionParser : AbstractParser
    {
        ...
        private void ParseConstValue(in Token constToken, ref Value? constValue)
        {
            ...
            {
                constValue = new FloatValue(GetDoubleValue(), Token.LineIndex, Token.StartColumnIndex);
            }
            else if (Token.Kind == TokenKind.String)
            {
                constValue = new StringValue(GetTokenName(), Token.LineIndex, Token.StartColumnIndex);
            }
            else
            {
        ...
        private Value? ParseFunctionBodyItem()
        {
            ...
            {
                body = new FloatValue(GetDoubleValue(), Token.LineIndex, Token.StartColumnIndex);
                MoveNext();
            }
            else if (Token.Kind == TokenKind.String)
            {
                body = new StringValue(GetTokenName(), Token.LineIndex, Token.StartColumnIndex);
                MoveNext();
            }
            else if (Token.Kind == TokenKind.Identificator)
            {
                ...
                {
                    body = new Variable(name, Token.LineIndex, Token.StartColumnIndex);
                }
                else if (_scope.IsConstExist(name))
                {
                    body = new NamedConstant(name, Token.LineIndex, Token.StartColumnIndex);
                }
                else if (_scope.IsFunctionExist(name))
                {
                    body = new Function(name, new List<Value>(), Token.LineIndex, Token.StartColumnIndex);
                }
                else
                {
        ...
        private Function? ParseFunctionCall(string funcName)
        {
            ...
            return new Function(funcName, argumentValues, Token.LineIndex, Token.StartColumnIndex);
        }
